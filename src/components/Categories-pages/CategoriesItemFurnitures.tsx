...
const CategoriesItemFurnitures = () => {
  ...
  // Define the structure of an 'Item' and filter items with the 'category' of 'furniture'
  interface Item {
    id: number;
    category: string;
    img: string;
  ...
  const filteredItems: Item[] = items.filter((item: Item) => item.category === "furniture");
    ...
    <section className='w-screen h-auto flex flex-col items-start justify-center'>
      <div className='mx-auto max-w-[1200px] px-[19px]'>
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 my-4'>
          {/* Map through the 'filteredItems' array to render individual furniture items */}
          {filteredItems.map((item) => (
            <div key={item.id} className='border-2 border-gray-400 hover:border-black p-2' onClick={() => setItemIDandCategory([item.id, item.category])}>
              <Link to={`/categories/products/${item.id}`}> {/* Create a link to the furniture item's detail page */}
                <div>
                  <img src={item.img} alt={item.description} /> {/* Display the furniture item's image with a description */}
                </div>
                <div>
                  <p className='font-medium'>{item.description}</p> {/* Display the furniture item's description */}
                  <p className='font-bold text-xl'>{item.price}$</p> {/* Display the furniture item's price */}
                </div>
              </Link>
            </div>
          ))}
